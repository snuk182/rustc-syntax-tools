...
use parse::ParseSess;
use syntex_pos::{symbol::Symbol, Span};
...
pub fn find_unwind_attr(diagnostic: Option<&Handler>, attrs: &[Attribute]) -> Option<UnwindAttr> {
    let syntex_error = |attr: &Attribute| {
        mark_used(attr);
        diagnostic.map(|d| {
...
/// Represents the #[stable], #[unstable], #[syntex_{deprecated,const_unstable}] attributes.
#[derive(RustcEncodable, RustcDecodable, Clone, Debug, PartialEq, Eq, Hash)]
pub struct Stability {
    pub level: StabilityLevel,
    pub feature: Symbol,
    pub syntex_depr: Option<RustcDeprecation>,
    pub syntex_const_unstable: Option<RustcConstUnstable>,
}
    ...
    where I: Iterator<Item = &'a Attribute>
{
    ...
    let mut syntex_depr: Option<RustcDeprecation> = None;
    let mut syntex_const_unstable: Option<RustcConstUnstable> = None;
    ...
    'outer: for attr in attrs_iter {
        ...
        if let Some(MetaItem { node: MetaItemKind::List(ref metas), .. }) = meta {
            ...
            match &*meta.name().as_str() {
                "syntex_deprecated" => {
                    if syntex_depr.is_some() {
                        span_err!(diagnostic, item_sp, E0540,
                                  "multiple syntex_deprecated attributes");
                        continue 'outer
                    }
                    ...
                    match (since, reason) {
                        (Some(since), Some(reason)) => {
                            syntex_depr = Some(RustcDeprecation {
                                since,
                                reason,
                            })
                        }
                ...
                "syntex_const_unstable" => {
                    if syntex_const_unstable.is_some() {
                        span_err!(diagnostic, item_sp, E0553,
                                  "multiple syntex_const_unstable attributes");
                        continue 'outer
                    }

                    get_meta!(feature);
                    if let Some(feature) = feature {
                        syntex_const_unstable = Some(RustcConstUnstable {
                            feature
                        });
                    } else {
    ...
    if let Some(syntex_depr) = syntex_depr {
        if let Some(ref mut stab) = stab {
            stab.syntex_depr = Some(syntex_depr);
        } else {
            span_err!(diagnostic, item_sp, E0549,
                      "syntex_deprecated attribute must be paired with \
                       either stable or unstable attribute");
        }
    }

    // Merge the const-unstable info into the stability info
    if let Some(syntex_const_unstable) = syntex_const_unstable {
        if let Some(ref mut stab) = stab {
            stab.syntex_const_unstable = Some(syntex_const_unstable);
        } else {
            span_err!(diagnostic, item_sp, E0630,
                      "syntex_const_unstable attribute must be paired with \
                       either stable or unstable attribute");
        }
    }
