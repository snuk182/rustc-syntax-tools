...
use syntex_pos::{self, Span, MultiSpan, BytePos, FileName, edition::Edition};
    ...
    fn next(&mut self) -> TokenAndSpan {
        loop {
            let tree = if !self.frame.open_delim {
                ...
                return TokenAndSpan { tok: token::Eof, sp: syntex_pos::DUMMY_SP }
            };
...
impl<'a> Parser<'a> {
    pub fn new(sess: &'a ParseSess,
        ...
        let mut parser = Parser {
            ...
            token_cursor: TokenCursor {
                frame: TokenCursorFrame::new(syntex_pos::DUMMY_SP, &Delimited {
                    delim: token::NoDelim,
                    tts: tokens.into(),
                }),
                stack: Vec::new(),
    ...
    fn parse_qpath(&mut self, style: PathStyle) -> PResult<'a, (QSelf, ast::Path)> {
        ...
        if self.eat_keyword(keywords::As) {
            ...
            path = ast::Path { segments: Vec::new(), span: syntex_pos::DUMMY_SP };
    ...
    crate fn process_potential_macro_variable(&mut self) {
        let (token, span) = match self.token {
            token::Dollar if self.span.ctxt() != syntex_pos::hygiene::SyntaxContext::empty() &&
    ...
    fn parse_item_mod(&mut self, outer_attrs: &[Attribute]) -> PResult<'a, ItemInfo> {
        ...
        if self.check(&token::Semi) {
            ...
            if in_cfg && self.recurse_into_file_modules {
                ...
                let placeholder = ast::Mod { inner: syntex_pos::DUMMY_SP, items: Vec::new() };
