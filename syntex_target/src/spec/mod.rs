...
impl Target {
    ...
    pub fn search(target_triple: &TargetTriple) -> Result<Target, String> {
        ...
        fn load_file(path: &Path) -> Result<Target, String> {
            ...
            let obj = from_reader(&mut &contents[..])
...
fn io_error_to_error(io: io::Error) -> json::ParserError {
    json::ParserError::IoError(io)
}
fn from_reader(rdr: &mut dyn io::Read) -> Result<Json, json::BuilderError> {
    let mut contents = Vec::new();
    match rdr.read_to_end(&mut contents) {
        Ok(c)  => c,
        Err(e) => return Err(io_error_to_error(e))
    };
    let s = match ::std::str::from_utf8(&contents).ok() {
        Some(s) => s,
